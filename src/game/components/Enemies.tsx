...
export function Enemies({ enemies, playerPositionRef, onEnemiesUpdate, pendingPlayerDamageRef }: Props) {
  ...
  useFrame((_, delta) => {
    ...
    const updated = enemies.map(e => {
      ...
      animTimers.current.set(e.id, (animTimers.current.get(e.id) || 0) + dt * (e.state === 'chase' ? 10 : 5));
      ...
      // Skip detailed updates for very distant enemies
      if (distSq > 10000) { // > 100 units away
        return e;
      }
      ...
      if (distToPlayer < e.attackRange) newState = 'attack';
      else if (distToPlayer < e.detectRange) newState = 'chase';
      else if (e.state === 'chase' || e.state === 'attack') newState = 'patrol';
      else if (e.state === 'idle') newState = 'patrol';
  ...
  return (
    <group>
      {enemies.map(e => {
        ...
        // Distance culling for rendering
        if (playerPos) {
        ...
        if (e.state === 'dead') {
          ...
          return (
            <group key={e.id} position={[e.position[0], e.position[1] - 0.5, e.position[2]]}>
              <mesh rotation={[Math.PI / 2, 0, 0]} geometry={boxGeo}
        ...
        return (
          <group key={e.id} position={[e.position[0], e.position[1] + bodyBob, e.position[2]]}>
            <group rotation={[0, faceAngle, 0]}>
              ...
              {/* Head */}
              <mesh position={[0, 0.15, 0.5]} geometry={boxGeo}
                scale={[0.28, 0.25, 0.3]} material={bodyMat} castShadow />
              <mesh position={[0, 0.1, 0.68]} geometry={boxGeo}
                scale={[0.18, 0.12, 0.15]} material={flash ? hitMat : wolfLightMat} castShadow />
              {/* Eyes */}
              <mesh position={[-0.08, 0.17, 0.64]} geometry={boxGeo}
